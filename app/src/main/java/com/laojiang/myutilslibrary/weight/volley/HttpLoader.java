...
import com.laojiang.utilslibrary.volley.utils.MyToast;
...
public class HttpLoader {
    ...
    public static ImageLoader getImageLoader() {
        if (mImageLoader == null) {
            mImageLoader = new ImageLoader(mRequestQueue, new LevelTwoLruBitmapCache(DemoApplication.getInstance()));
        }
        return mImageLoader;
    }
    ...
    private static void request(int method, String url, Map<String, String> params, Class<? extends RBResponse> clazz, final int requestCode, final ResponseListener listener, boolean isCache) {
        if (mInFlightRequests.get(requestCode) == null) {
            ...
            if (method == Request.Method.GET) {
                request = makeRBRequest(method, url + buildGetParam(params), null,clazz, requestCode, listener, isCache);
            } else {
                request = makeRBRequest(method, url, params, clazz,requestCode, listener, isCache);
            }
    ...
    public interface ResponseListener {
         ...
         */
        void onGetResponseError(int requestCode);
    }
    ...
    private static class RequestListener implements Response.ErrorListener, Response.Listener<RBResponse> {
        ...
        public void onErrorResponse(VolleyError error) {
            ...
            if (listener != null) {
                listener.onGetResponseError(requestCode);
            }
            boolean networkAvailable = NetUtils.isNetworkAvailable((DemoApplication.getInstance()));
            if (networkAvailable){
                MyToast.show(DemoApplication.getInstance(), "服务器正在升级请稍等");
            }else {
                MyToast.show(DemoApplication.getInstance(), "请检查您的网络");
            }
        ...
        public void onResponse(RBResponse response) {
            ...
            if (response != null) {
                ...
                ALog.d("我是code-="+response.getCode());
                if (response.getCode()!=1) {
                    MyToast.show(DemoApplication.getInstance(), response.getMessage());
                    listener.onGetResponseError(response.getCode());
                    return;
                }
        ...
        private void getCacheResponse(Request request) {
            try {
                //获取缓存文件
                File cacheFile = new File(DemoApplication.getInstance().getCacheDir(), "" + MD5Utils.encode(request.getUrl()));
